...
function ocrProfileSetting(profile) {
  const supported = ["generic", "deepseek_ocr", "deepseek_ocr_llamacpp", "glm_ocr"];
  const selected = supported.includes(profile) ? profile : "generic";
  return settingInput("ocr_profile", "Request profile", selected, {
    type: "select",
    full: true,
    choices: [["generic", "Generic vision model"], ["deepseek_ocr", "DeepSeek OCR-2 via vLLM"], ["deepseek_ocr_llamacpp", "DeepSeek OCR-2 GGUF via llama.cpp"], ["glm_ocr", "GLM-OCR via vLLM"]],
    note: "Specialist profiles send each model's short native OCR command without a system prompt."
  });
}
  ...
  content.innerHTML = `<section class="page-intro"><div><h2>Settings</h2><p>Configure Paperless, model endpoints, automation, notifications, and interface options. Save changed values before testing a connection.</p></div></section>
    <div class="settings-layout">
      ...
      <form class="settings-form" id="settings-form">
        ...
        <section class="panel settings-section" id="settings-ocr"><header class="panel-head"><div><h3>Vision OCR model</h3><p class="section-description">Receives one rendered page image per request through the shared API.</p></div><button class="button ghost small" type="button" data-action="test-connection" data-target="ocr">Test OCR model</button></header><div class="panel-body"><div class="form-grid">${settingInput("ocr_model", "Model name", s.ocr_model, { full: true })}${ocrProfileSetting(s.ocr_profile)}<div class="ocr-preference">${settingCheck("prefer_clerk_ocr", "Prefer Clerk OCR after a trusted match", "Applies to every OCR profile after Clerk confirms that its complete result agrees with existing Paperless OCR.", s.prefer_clerk_ocr)}</div>${settingInput("ocr_context_tokens", "Context limit", s.ocr_context_tokens, { type: "number", min: 1024 })}${settingInput("ocr_max_output_tokens", "Maximum output tokens", s.ocr_max_output_tokens, { type: "number", min: 256 })}</div></div></section>
